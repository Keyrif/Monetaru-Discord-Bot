...
return message.channel.send(`<:bump:909822887404380240> Asteapta fraticu **${remaining}** ca sa muncesti iar.`)
        .catch(console.error);
    }
    ...
    message.channel.send(embed).then(() => {
      message.channel.awaitMessages(filter, { max: 1, time: 10000, errors: ['time'] })
        .then(collected => {
          ...
          .setTitle('Work')
          .setThumbnail('https://i.imgur.com/qTfipOV.png')
          .addField(`Ai muncit ca ultimul sclav si ai primit:`, `+ ${coins} <:newccoin:911788039603359774>`, true)
          .setFooter(`${message.author.tag}`, `${message.author.displayAvatarURL({ dynamic: true, size: 1024 })}`)
